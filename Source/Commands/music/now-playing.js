...
module.exports = {
  name: "nowplaying",
  aliases: ["np", "suancalan", "sc"],
  run: async (client, message, args) => {
    const Channel = message.member.voice.channel;

    if (!Channel) return message.channel.send("komutu çalıştırmadan önce bir ses kanalına girmen gerekiyor.");

    const Queue = await client.queue.get(message.guild.id);

    if (!Queue)
      return message.channel.send(
        "Şimdi hiçbir şey çalmıyor. Bunun için şarkı eklemelisin :"
      );
    ...
    const Data = `Şarkı - **[${Song.Title}](${Song.Link})**\nYayınlayan - **[${
      Song.Author
    }](${Song.AuthorLink})**\nYükleyen - **${
      Song.Upload
    }**\nGörüntülenme - **${Song.Views ||
      0}**\nDakika - **${Total}**\nRemaining - **${Remaining}**\n\n`;
    ...
    const Embed = new Discord.MessageEmbed()
      ...
      .setFooter(`${Song.Owner} tarafından eklendi.`)
      .setTimestamp();
